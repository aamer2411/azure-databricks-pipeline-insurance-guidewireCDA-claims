...
![ADF Master Pipeline](screenshots/pl_m_source_to_refined.png)

### Unity Catalog - Registered Tables

![Databricks Unity Catalog](screenshots/databricks_unity_catalog.png)

### ADLS Storage Structure

The `insurance-claims-domain` container holds four zones. Each raw table folder follows the `{table}/{fingerprint}/{timestamp}/` hierarchy; replica and refined zones contain external Delta tables.
...
### Azure SQL - ABC Metadata Database

The `abc` schema in Azure SQL holds all pipeline control and run-history tables that power the metadata-driven execution model.

![Azure SQL ABC Metadata Database](screenshots/azure_sql.png)

See [screenshots/README.md](screenshots/README.md) for all pipeline canvas diagrams, linked services, datasets, and workspace screenshots.
...
│
└── screenshots/                            ADF pipeline canvases, Databricks workspace, Unity Catalog
    ├── README.md                           Annotated index of all screenshots
    ├── pl_m_source_to_refined.png
    ├── pl_c_source_to_raw.png
    ├── pl_c_raw_to_replica.png
    ├── pl_c_replica_to_refined.png
    ├── pl_gc_relica_data_validation.png
